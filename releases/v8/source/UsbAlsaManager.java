...
public final class UsbAlsaManager {
    ...
    // piBrick internal C-Media USB audio codec.
    // This device is soldered inside the unit and must not be exposed by
    // UsbAlsaManager as an external USB headset.
    private static final int USB_VENDORID_PIBRICK_AUDIO = 0x0D8C;
    private static final int USB_PRODUCTID_PIBRICK_AUDIO = 0x0014;
    ...
    static final List<DenyListEntry> sDeviceDenylist = Arrays.asList(
            new DenyListEntry(USB_VENDORID_PIBRICK_AUDIO,
                    USB_PRODUCTID_PIBRICK_AUDIO,
                    USB_DENYLIST_OUTPUT),
    ...
    public void systemReady() {
        ...
        // Start observing first, then seed the set with ALSA nodes that already
        // existed before system_server started. alsaFileAdded() is idempotent.
        mAlsaObserver.startWatching();

        File[] alsaFiles = new File(ALSA_DIRECTORY).listFiles();
        if (alsaFiles != null) {
            for (File alsaFile : alsaFiles) {
                alsaFileAdded(alsaFile.getName());
            }
        }
    }
